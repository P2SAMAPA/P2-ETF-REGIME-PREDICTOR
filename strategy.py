"""
strategy.py — P2-ETF-REGIME-PREDICTOR v2
=========================================
...
# Default ETF list — Option A. Overridden by passing target_etfs to each fn.
...
def execute_strategy(
    ...
    target_etfs    : ETF universe list. Defaults to Option A if not provided.
    ...
    if target_etfs is None:
        target_etfs = _DEFAULT_TARGET_ETFS
...
def calculate_metrics(strat_rets: np.ndarray,
    ...
    daily_rf = rf_rate / 252
    excess   = strat_rets - daily_rf
    sharpe   = (float(np.mean(excess)) /
                (float(np.std(excess)) + 1e-9) * np.sqrt(252))
    tol          = daily_rf * 0.01
